...
func rotate8OrZero(input, count byte) byte {
	if count == 0 {
		return 0
	}
...
func (v sapBytes) at(i byte) byte { return v[i%byte(len(v))] }
...
func (state *sapState) nonlinearCircuit() {
	hash, work := sapBytes(state.hash[:]), sapBytes(state.work[:])
	matrix, seed := sapBytes(state.matrix[:]), sapBytes(sapSeed[:])
	aux := &state.aux
	// h/m/s read hash/matrix/seed through work; ma reads matrix through aux.
	h := func(i int) byte { return hash.at(work[i]) }
	m := func(i int) byte { return matrix.at(work[i]) }
	s := func(i int) byte { return seed.at(work[i]) }
	ma := func(i int) byte { return matrix.at(aux[i]) }
	...
	aux[4] = 92 - m(32)
	aux[9] = m(15) + 0x9e
	work[34] += seed.at(aux[9]) / 5
	hash[19] += 0xe6 ^ hash.at(aux[9])>>1&0x66
	work[15] ^= 3*rotate8OrZero(work[72], -s(190)&7) - 9*s(126)
	hash[15] ^= cube(m(181))
	matrix[4] ^= work[202] / 3
	matrix[1] += cube(majority(92-hash.at(aux[4]), ^work[105], 0xc6))
	hash[19] ^= byte(int(224|s(92)&27) * int(m(41)) / 3)
	work[140] += rotate8OrZero(92, -work[5]&7)
	matrix[12] += majority(^work[4]^m(12), work[182], 192)
	work[36] += 125
	work[124] = bits.RotateLeft8(majority(majority(work[138], hash[15], 74), h(43), 95), 4)
	auxHash := hash.at(aux[9])
	...
	hash[14] ^= ma(2)
	work[19] += majority(rotate8OrZero(seed.at(h(201)), (m(112)<<1)&7),
		((h(208)&^0x7c)|(h(164)&0x7c))/5, 37)
	matrix[8] = rotate8OrZero(140, -square(s(45))&7) ^ aux[4]
	work[190] = 56
	work[53] = ^((h(83) | 204) / 5)
	hash[13] += h(41)
	...
	aux[7] = seedBits/3 - ma(9) -
		(0x14 | (work[151]|aux[4]&0x22)&0x62 | aux[4]&work[151]&0x88)
	expandedSelector := expansionBits ^ ((aux[4] >> 1) & 101) ^ 75
	...
	wide := int(selectBits(71, matrix.at(expandedSelector), seed.at(aux[5])))
	...
	matrix[22] = majority(selectBits(hash[14]|28, (work[7]&28)|0x82, h(93)),
		rotate8OrZero(ma(4), rotate8OrZero(work[11], -m(28)&7)&7), matrix[33]) + 74
	hash[15] -= majority(majority(aux[3], aux[4], 214), seed.at(h(39)^217), aux[6])

	hash9 := hash.at(aux[9])
	indexedHash := hash.at(((aux[4] / 3) - (aux[9] | work[22])) ^ aux[6] ^
		(((m(57) | hash9) & (0x52 | (aux[9] & 0x0d))) | ((m(57)&hash9 | aux[9]) & 0x20)))
	aux[6] = square(square(h(99))) | ma(9)
	aux[1] += rotate8OrZero(h(151)|s(202), h(50)&7) +
	...
	aux[0] = selectBits(hash[10]&7, ma(6)&h(209),
		selectBits(0x47, rotate8OrZero(s(127), ma(6)&7), seed.at(ma(5))<<1))
	selectedSquare := selectBits(198, square(m(14)), h(145)^aux[0])
	seed9 := seed.at(aux[9])
	matrix[2] += ((hash.at(aux[3]) << 1) & ((work[25] & 0x96) | (seed9 & 8))) | (seed9 & 0x40)
	matrix[14] -= selectBits(34, work[97], ma(3)&(aux[0]^m(100)))
	work[23] ^= majority(majority(s(17), hash.at(aux[3]), aux[0]), work[50]/3, 0x76) << 1
	hash[17] = 115
	hash[13] = majority(hash.at(aux[7]), work[10], 82)>>1&0x68 | h(39)&0x17
	matrix[33] -= work[113] & 9
	matrix[28] -= aux[3]&^0x20 | work[110]>>1&0x20
	work[95] = seed.at(aux[3])
	hash[15] = majority(work[95]-48, ^work[184], 189) & cube(majority(aux[7], seed.at(aux[1]), 0xaa))
	matrix[22] += work[183]
	aux[4] ^= 3 * s(1)
	aux[5] += 198 * majority(s(178), ma(1), 209) * h(13) * (s(26) >> 1)
	aux[8] = selectBits(10, ma(3), ma(9))
	matrix[18] -= selectBits(hash[15], aux[5]/15, cube(hash.at(aux[6])|81))
	aux[1] += seed.at(hash.at(aux[1]))/3 - h(160)
	...
	work[5] = rotate8OrZero(0x38, -(h(61)/5)&7) ^ (^ma(8))/5
	work[198] += work[3]
	wide = int(162 | ma(9))
	work[164] += byte(wide * wide / 5)
	aux[2] = majority(rotate8OrZero(139, -aux[5]&7), hash.at(aux[3]), 12) |
		selectBits(95, cube(seed9), hash.at(aux[7]))
	matrix[12] += ((work[103] & 32) | (aux[2] & (work[103] | 60)) | 16) / 3
	work[143] -= 0x12 | (selectBits(aux[9], selectBits(matrix[8], work[35], aux[7]), aux[8]/3) &
		(0x4d | work[172]>>1&0x20))
	matrix[29] = 162
	hash[15] += majority(m(149)^square(work[43]), selectBits(95, h(125), seed.at(aux[1]))>>1, 115)
	aux[9] -= hash.at(aux[7])
	hash[7] -= square(rotate8OrZero(ma(5), -m(17)*(m(17)&1)))
	matrix[8] += cube(s(202)) - work[184]
	hash[16] = m(102) << 1 & 0x84
	aux[6] ^= seed.at(aux[7]) >> 1
	hash[7] -= h(191) - selectBits(177, seed.at(seed.at(aux[1])), s(80)<<1)
	hash[6] = h(119)
	hash[12] = (hash.at(aux[8]) ^ (m(71) + m(15))) &
		majority((work[118]&^0x2e)|2, square(hash.at(aux[9])), 27)
	digestIndex := selectBits(0xa9, s(57)*231, majority(work[32], ma(1), 23)) / 5
	seedSample := seed.at(aux[6])
	aux[5] = majority((seedSample&0x1c)|(h(82)&0xa2)|(seed.at(digestIndex)&0x41),
		majority(cube(hash.at(aux[7])), work[82], 92), 192) ^ digestIndex
	matrix[25] ^= 2*hash.at(aux[9])*work[5] - rotate8OrZero(aux[4], seedSample&7)&(aux[3]+110)
}
