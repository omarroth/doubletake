...
// SAPHash computes the 16-byte PlayFair SAP hash used by the FairPlay
// exchange. blockIn must contain at least one 64-byte block and keyOut must
// have room for 16 bytes.
func SAPHash(blockIn []byte, keyOut []byte) {
	sapHash(blockIn, keyOut)
}
...
// DecryptSAPMessage applies the PlayFair SAP message decrypt step used before
// session-key derivation. messageIn must contain the 144-byte FPLY message and
// decryptedOut must have room for 128 bytes.
func DecryptSAPMessage(messageIn []byte, decryptedOut []byte) {
	decryptMessage(messageIn, decryptedOut)
}
